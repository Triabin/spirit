...
| [Java SE 24.0](https://www.oracle.com/java/technologies/javase/jdk24-archive-downloads.html) |                      | 2025/3/20  | 1、密钥派生函数 API（预览）<br />2、提前类加载和链接<br />3、类文件API<br />4、流收集器<br />5、永久禁用安全管理器<br />6、作用域值（预览）<br />7、虚拟线程的同步而不固定平台线程<br />8、在没有JMOD文件的情况下链接运行时镜像<br />9、简化的源文件和实例主方法<br />10、量子抗性数字签名算法（ML-DSA）<br />11、使用`sun.misc.Unsafe`内存访问方法时发出警告<br />12、结构化并发 |
| [Java SE 25.0 LTS](https://www.oracle.com/java/technologies/javase/jdk25-archive-downloads.html) |                      | 2025/9/16  | 1、作用域值<br />2、紧凑源文件与实例主方法<br />3、紧凑对象头<br />4、分代Shenandoah GC<br />5、模式匹配支持基本类型（第三次预览）<br />6、模块导入声明<br />7、灵活的构造函数体<br />8、向量API，第十次孵化 |
