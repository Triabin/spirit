...
<img src="https://gitee.com/triabin/img_bed/raw/master/2026/05/20/02a32e05d9207c8d6614eb7009049132-image-20250419144233896.png" align="left"/>

<div style="clear: both;"></div>

如上图，从内到外分别是内容（content）、内边距（padding）、边框（border）、外边距（margin），CSS样式主要就是用来设定这几个属性的样式。除了这几个部分以外，有些时候还会有滚动条（scroll-bar）。

CSS样式设置的width和height值具体包含哪些内容主要有以下几种情况：

* `box-sizing: content-box`（默认情况）：`size = content`，主要含义就是元素内容能占用的总尺寸，其他margin、border、padding的值会额外往外撑开，所以实际占用的尺寸是`size = content + padding + border + margin`。
* `box-sizing: border-box`：`size = content + padding + border`，主要是排除了外边距以外的占用尺寸。

### 1.1 DOM对象尺寸属性值与盒模型对应关系
